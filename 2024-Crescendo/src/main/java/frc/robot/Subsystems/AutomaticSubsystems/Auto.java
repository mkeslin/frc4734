...
public class Auto {
    ...
    private void alignAprilTag() {
        if (Limelight.getPipeline() != 0) {
            ...
            if (/*Timer.getMatchTime() < 14.0 &&*/timer.get() < 3) {
                forward =
                    (
                        (Limelight.getArea() > 1.5)
                            ? -0.5
                            : (
                                (Limelight.getArea() < 0.5 && Limelight.getArea() > 0)
                                    ? Math.min(1, 1 / (2 * Limelight.getArea()))
                                    : 0
                            )
                    );
                strafe =
                    (Math.abs(Limelight.getX()) > 3 && Limelight.getArea() < 2)
                        ? Limelight.getX() / 20
                        : 0;
    ...
    private void alignCube() {
        if (Limelight.getPipeline() != 1) {
            ...
            if (minY > -13) {
                if (
                    timer.get() - colorTimeelapsed > 0.125 && Math.abs(oldY - Limelight.getY()) < 3
                ) {
                    forward =
                        (Limelight.getArea() > 6)
                            ? -0.5
                            : (
                                (Limelight.getArea() > 0.2)
                                    ? Math.min(0.6, 0.6 / (Limelight.getArea()))
                                    : 0
                            );
                    strafe =
                        (Limelight.getX() < -5 || Limelight.getX() > 5) ? Limelight.getX() / 55 : 0;
    ...
    public void scoreHigh() {
        ...
        if ((timer.get() < 0.1) || (timer.get() < 2 && horizontalElevator.getElevatorMovingOut())) {
            ...
            (timer.get() > 2.5 && timer.get() < 2.6) || (horizontalElevator.getElevatorMovingIn())
        ) {
        ...
        if ((timer.get() < 0.1) || (timer.get() < 2 && verticalElevator.getElevatorMovingOut())) {
            ...
            (timer.get() > 2.5 && timer.get() < 2.6) || (verticalElevator.getElevatorMovingIn())
        ) {
    ...
    private void turnAround() { //add in turn left/right cases based on AprilTag ID
        ...
        if (
            Math.abs(swerve.getSwervePosition() - startPos) >= 62000 && !rotateArm.getArmMovingOut()
        ) {
            changeState();
        }
    }
