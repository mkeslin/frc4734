...
public class SwerveDrive {
  ...
  private void swerveDrive(TalonFX dm, TalonFX sm, double a, double s) {
    if (Math.abs(s) * DTS > 0.05)
      ...
      dm.setControl(new DutyCycleOut(s * DTS * SpeedFactor));
    else
      // dm.set(ControlMode.PercentOutput, 0.0);
      dm.setControl(new DutyCycleOut(0.0));
    if (Math.abs(a) / 180 > 0.05)
      // sm.set(ControlMode.PercentOutput, a / 180.0);
      dm.setControl(new DutyCycleOut(a / 180.0));
    else
  ...
  private void calculateEncoder() {
    ...
    FRA = fixAngle(FRS.getPosition().getValueAsDouble() - FREO);
    FLA = fixAngle(FLS.getPosition().getValueAsDouble() - FLEO);
    BRA = fixAngle(BRS.getPosition().getValueAsDouble() - BREO);
    BLA = fixAngle(BLS.getPosition().getValueAsDouble() - BLEO);
  }
  ...
  public void zeroMotors() {
    FRD.setControl(new DutyCycleOut(0.0));
    FLD.setControl(new DutyCycleOut(0.0));
    BRD.setControl(new DutyCycleOut(0.0));
    BLD.setControl(new DutyCycleOut(0.0));
    FRS.setControl(new DutyCycleOut(0.0));
    FLS.setControl(new DutyCycleOut(0.0));
    BRS.setControl(new DutyCycleOut(0.0));
    BLS.setControl(new DutyCycleOut(0.0));
  ...
  public void setMode(NeutralModeValue n) {
