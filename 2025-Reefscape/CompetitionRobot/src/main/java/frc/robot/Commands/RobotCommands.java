...
// import frc.robot.Constants.Constants.ElevatorPosition;
...
public class RobotCommands {
    ...
    public static Command autoPrepareCoralScoreCommand(ScoreLevel level, Elevator elevator, Arm arm, CoralSim coralSim) {
        ElevatorPosition elevatorPosition;
        ArmPosition armPosition;
        switch (level) {
            case L1 -> {
    ...
    public static Command scoreCoralCommand(CommandSwerveDrivetrain drivetrain, Elevator elevator, Arm arm, CoralSim coralSim) {
        Map<ScoreLevel, Command> commandMap = Map.ofEntries(
                Map.entry(
                        ScoreLevel.L1, Commands.parallel(
                                drivetrain.moveVoltageTimeCommand(4, 0.5),
                                elevator.movePositionDeltaCommand(() -> ElevatorConstants.SCORING_MOVEMENT)
                                        .asProxy())),
                Map.entry(
                        ScoreLevel.L2,
                        Commands.parallel(
